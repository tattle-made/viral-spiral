defmodule ViralSpiralWeb.GameRoomChannel.StateAdapter do
  ...
  def make_turn_player(%State{} = state) do
    current_player_id = state.turn.current
    current_turn_player = state.players[current_player_id]
    %{id: current_player_id, name: current_turn_player.name}
  end
  ...
  # JSON-serializable cancel — no FormData
  def make_cancel(state, current_player_id) do
    ...
    affinity_options =
      make_options(affinities, cancel_threshold)
      |> Enum.map(&%{value: &1.type, label: Affinity.label(&1.type), polarity: &1.polarity})
    ...
    from_player = !is_nil(from_player_id) && %{id: from_player_id, name: state.players[from_player_id].name}
    target_player = !is_nil(target_player_id) && %{id: target_player_id, name: state.players[target_player_id].name}
  ...
  def make_options(affinities, cancel_threshold) do
    Enum.map(affinities, fn {k, v} ->
      ...
      %{type: k, polarity: polarity, can_cancel: abs(v) >= cancel_threshold}
    end)
  ...
  def make_power_viral_spiral(state, player_id) do
    ...
    player = state.players[player_id]
    bias_enabled = player.biases |> Map.values() |> Enum.any?(&(&1 >= threshold))
    affinity_enabled = player.affinities |> Map.values() |> Enum.any?(&(abs(&1) >= threshold))
    enabled = (bias_enabled or affinity_enabled) && State.current_turn_player(state).id == player_id
    %{enabled: enabled}
  end
    ...
    case state.room.state do
      :over ->
        case State.game_over_status(state) do
          {:over, :world, data} -> Template.generate_game_over_message(data)
          {:over, :player, data} -> Template.generate_game_over_message(data)
        end
      _ -> nil
    end
  end

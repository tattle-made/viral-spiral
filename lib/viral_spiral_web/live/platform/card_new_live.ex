defmodule ViralSpiralWeb.Platform.CardNewLive do
                    ...
                    <%= case field.type do %>
                      <% "boolean" -> %>
                        <input
                          ...
                          checked={get_in(@attributes, [field.name]) in [true, "true"]}
                      ...
                      <% "enum" -> %>
                        ...
                        >
                          ...
                          <%= for opt <- (field.enum_values || []) do %>
                            <option value={opt} selected={get_in(@attributes, [field.name]) == opt}>
                              <%= opt %>
                            </option>
                          <% end %>
                        </select>
    ...
    if map_size(errors) > 0 do
      ...
      image_key =
        consume_uploaded_entries(socket, :image, fn %{path: path}, entry ->
          key = S3.card_image_key(card_id, entry.client_name)
      ...
      card_attrs = %{
        ...
        "card_schema_id" => socket.assigns.schema_id,
        "created_by" => socket.assigns.current_user.id
      }

      case Platform.create_card(card_attrs) do
        {:ok, _card} ->
          {:noreply,
           socket
